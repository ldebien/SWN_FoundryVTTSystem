export const augmentTable = (table, html, data) => {
  ...
  const flag = table.object.getFlag("swn", "treasure");
  const treasure = flag
  ...
  html.find(".toggle-treasure").click((ev) => {
    let isTreasure = table.object.getFlag("swn", "treasure");
    table.object.setFlag("swn", "treasure", !isTreasure);
  });
  ...
  if (flag) {
    ...
    // Replace Roll button
    const roll = `<button class="roll-treasure" type="button"><i class="fas fa-gem"></i> ${game.i18n.localize('SWN.table.treasure.roll')}</button>`;
...
async function rollTreasure(table, options = {}) {
  ...
  let html = await renderTemplate(
    "systems/swn/templates/chat/roll-treasure.html",
    templateData
  );

  let chatData = {
    content: html,
    // sound: "/systems/swn/assets/coins.mp3"
  }
